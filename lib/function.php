...
function get_land_code(){
	$query = mysqli_query("select land_code from counters");
...
function edit_land_code(){
	mysqli_query("update counters set land_code = land_code + 1");
}
function get_land_area($land_id){
	$query = mysqli_query("select sum(farmer_land_area) as jumlah from farmer_lands where land_id = '$land_id'");
...
function get_event_code(){
	$query = mysqli_query("select event_code from config");
	$row = mysql_fetch_object($query);
...
function get_user_data(){
	global $mysqli;
	$query_user = mysqli_query($mysqli, "select * from users where user_id = '".$_SESSION['user_id']."'");
...
function get_urutan($parent_id, $number){
	$query = mysqli_query("select count(vps_id) as result from village_profile_structures where vps_parent_id = '$parent_id' and vps_number < '$number'");
