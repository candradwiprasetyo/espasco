...
switch ($page) {
	...
	case 'list':
		...
		if(isset($_GET['preview'])){
			...
			$total_pajak = (get_total_pajak($date1, $date2));

			$total_pokok = get_total_pokok($date1, $date2);
			$total_pembelian = get_total_pembelian($date1, $date2);
			$total_biaya = number_format(($total_pokok + $total_pembelian), 2);

			$total_pajak = number_format(($total_pajak - $total_pembelian), 2);
